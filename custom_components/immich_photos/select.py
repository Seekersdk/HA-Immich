...
async def async_setup_entry(
    ...
    for coordinator in coordinators.values():
        ...
        entities.append(ImmichDateFilterModeSelect(coordinator, entry))
...
class ImmichCropModeSelect(_ImmichBaseSelect):
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        # Re-render current image with new crop mode
...
class ImmichDateFilterModeSelect(_ImmichBaseSelect):
    _attr_options = DATE_FILTER_MODES
    _attr_icon = "mdi:calendar-filter"

    def __init__(self, coordinator, entry):
        super().__init__(coordinator, entry, "date_filter_mode", "Date Filter")

    @property
    def current_option(self) -> str:
        return self.coordinator.date_filter_mode

    async def async_select_option(self, option: str) -> None:
        self.coordinator.date_filter_mode = option
        # Invalidate pool so it's re-fetched with new filter
        await self.coordinator.async_force_refresh_pool()
        await self.coordinator.async_next_media()
        self.async_write_ha_state()
